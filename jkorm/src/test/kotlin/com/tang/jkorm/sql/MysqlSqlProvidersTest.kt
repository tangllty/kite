...
class MysqlSqlProvidersTest : SqlProviderTest {
    ...
    @Test
    override fun paginateLimit() {
        val account = Account(username = "tang")
        val statement = sqlProvider.paginate(Account::class.java, account, arrayOf(OrderItem("id", false)), 1, 5)
        equals("select $columns from account where username = ? order by id desc limit ?, ?", statement.sql)
        equals("select $columns from account where username = 'tang' order by id desc limit 0, 5", statement.getActualSql())
    }
